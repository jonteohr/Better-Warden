...
public Action OnPlayerTeamChat(int client, char[] command, int args) {
	...
	for(int i = 1; i <= MaxClients; i++) {
		...
		if(!IsValidClient(i))
			continue;	
		SQL_GetGang(i, buff, sizeof(buff));
		
		if(!StrEqual(buff, gang, false))
			continue;
		
		CPrintToChat(i, "{green}(%s) %N :{default} %s", gang, client, final);
	}
...
public void DeleteGang(int client) { // Deletes gangs
	if(SQL_OwnsGang(client)) {
		...
		if(SQL_DeleteGang(gangID)) {
			...
			CPrintToChat(client, "%s %t", g_sPrefix, "Failed Delete Gang", gang);
		}
		
	} else {
		CPrintToChat(client, "%s %t", g_sPrefix, "Not Owner"); // Not the owner
	}
}
